...
private:
    ...
    int _n_opens = 0;
    bool _permanent;
    // _sectors stored in memory of the files (only the home node is forced to be up to date)
...
public:
    ...
    Capio_file(const std::string_view& committed, const std::string_view& mode,
               bool directory, long int n_files_expected, bool permanent, std::size_t init_size,
    ...
    Capio_file(bool directory, bool permanent, std::size_t init_size, long int n_close_expected = -1) :
            _buf_size(init_size),
            _committed("on_termination"),
            _directory(directory),
            _n_close_expected(n_close_expected),
    ...
    inline void add_fd(int tid, int fd) {
    ...
    [[nodiscard]] inline size_t get_buf_size() const {
        return _buf_size;
    }

    [[nodiscard]] inline std::string_view get_committed() const {
        return _committed;
    }
    ...
    [[nodiscard]] inline std::string_view get_mode() const {
        return _mode;
    }
    ...
    [[nodiscard]] inline bool is_deletable() const {
        return _n_opens == 0 && _n_links <= 0;
    }

    [[nodiscard]] inline bool is_dir() const {
        return _directory;
    }
