...
void update_metadata_conf(std::string &path, size_t pos, long int n_files, size_t batch_size, const std::string &committed,
    ...
    START_LOG(gettid(), "call(path=%s, pos=%ld, n_files=%ld, batch_size=%ld, committed=%s, mode=%s, app_name=%s, permanent=&s, n_close=%ld)",
              path.c_str(), pos, n_files, batch_size, committed.c_str(), mode.c_str(), app_name.c_str(),
              permanent? "true" : "false", n_close);
