...
CapioFile &create_capio_file(const std::filesystem::path &path, bool is_dir, size_t init_size) {
    ...
    if (it == metadata_conf.end()) {
        ...
        if (pos == -1) {
            ...
            if (backend->store_file_in_memory()) {
                c_file = new CapioMemFile(path, is_dir, false, init_size);
            } else {
                c_file = new CapioFsFile(path, is_dir, false, init_size);
            }
            ...
            if (backend->store_file_in_memory()) {
                c_file = new CapioMemFile(path, committed, mode, is_dir, n_files, permanent,
                                          init_size, n_close);
            } else {
                c_file = new CapioFsFile(path, committed, mode, is_dir, n_files, permanent,
                                         init_size, n_close);
            }
        ...
        LOG("Creating new capio_file. committed=%s, mode=%s", committed.c_str(), mode.c_str());
        if (backend->store_file_in_memory()) {
            c_file = new CapioMemFile(path, committed, mode, is_dir, n_files, permanent, init_size,
                                      n_close);
        } else {
            c_file = new CapioFsFile(path, committed, mode, is_dir, n_files, permanent, init_size,
                                     n_close);
        }
