...
/*
 * It contains all the read requested by other nodes for which the data is not yet available
...
void capio_server(int rank) {
    static const std::array<CSHandler_t, CAPIO_NR_REQUESTS> request_handlers = build_request_handlers_table();
...
inline void serve_remote_read(const char *path_c, int dest, long int offset, long int nbytes, int complete) {
    START_LOG(gettid(), "call(pach_c=%s, dest=%d, offset=%ld, nbytes=%ld, complete=%d)", path_c, dest, offset, nbytes, complete);
...
void wait_for_data(char * const path, off64_t offset, int dest, off64_t nbytes, sem_t *sem) {
    START_LOG(gettid(), "call(path=%s, offset=%ld, dest=%d, nbytes=%ld)", path, offset, dest, nbytes);
    if (sem_wait(sem) == -1)
...
void wait_for_n_files(char * const prefix, std::vector<std::string> *files_path, size_t n_files, int dest, sem_t *sem) {
...
std::vector<std::string> *files_avaiable(const std::string& prefix, const std::string& app, const std::string& path_file, int n_files) {
    START_LOG(gettid(), "call(prefix=%s, app=%s, path_file=%s, n_files=%d)", prefix.c_str(), app.c_str(), path_file.c_str(), n_files);
...
int parseCLI(int argc, char **argv, int rank){
    ...
    args::ValueFlag<std::string> logfile_src(arguments, "filename", CAPIO_SERVER_ARG_PARSER_LOGILE_OPT_HELP, {'l', "log"});
    args::ValueFlag<std::string> config(arguments, "filename", CAPIO_SERVER_ARG_PARSER_CONFIG_OPT_HELP, {'c', "config"});
    ...
    if (logfile_src) {
        ...
        if (token.find(".log") != std::string::npos)
            token.erase(token.length() - 4); //delete .log if for some reason is given as parameter
