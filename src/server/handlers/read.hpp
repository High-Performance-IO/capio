...
inline void handle_pending_read(int tid, int fd, long int process_offset, long int count, bool is_getdents) {
    START_LOG(tid, "call(tid=%d, fd=%d, process_offset=%ld, count=%ld, is_getdents=%s)",
              tid, fd, process_offset, count, is_getdents? "true" : "false");
...
inline void handle_local_read(int tid, int fd, off64_t count, bool dir, bool is_getdents, bool is_prod) {
    START_LOG(tid, "call(tid=%d, fd=%d, count=%ld, dir=%s, is_getdents=%s, is_prod=%s)",
              tid, fd, count, dir? "true" : "false", is_getdents? "true" : "false",
              is_prod? "true" : "false");
...
inline bool read_from_local_mem(int tid, off64_t process_offset, off64_t end_of_read,
    ...
    START_LOG(tid, "call(tid=%d, process_ofset=%ld, end_of_read=%ld, end_of_sector=%ld, count=%ld, path=%s)",
...
inline void handle_remote_read(int tid, int fd, off64_t count, int rank, bool dir, bool is_getdents) {
    START_LOG(tid, "call(tid=%d, fd=%d, count=%ld, rank=%d, dir=%s, is_getdents=%s)",
              tid, fd, count, rank, dir? "true" : "false", is_getdents? "true" : "false");
...
inline void wait_for_file(int tid, int fd, off64_t count, bool dir, bool is_getdents) {
    START_LOG(gettid(), "call(tid=%d, fd=%d, count=%ld, dir=%s, is_getdents=%s)",
              tid, fd, count, dir? "true": "false", is_getdents? "true" : "false");
    ...
    if (strcmp(std::get<0>(get_file_location(path_to_check.data())), node_name) == 0) {
        ...
        if (!c_file.complete) {
            ...
            if (res) {
                const std::lock_guard<std::mutex> lg(pending_remote_reads_mutex);
                pending_remote_reads[path_to_check.data()].emplace_back(tid, fd, count, is_getdents);
            }
        }
...
inline void handle_read(int tid, int fd, off64_t count, bool dir, bool is_getdents, int rank) {
    START_LOG(gettid(), "call(tid=%d, fd=%d, count=%ld, dir=%s, is_getdents=%s, rank=%d)",
              tid, fd, count, dir? "true" : "false", is_getdents? "true" : "false", rank);
...
inline void solve_remote_reads(size_t bytes_received, size_t offset, size_t file_size, const char *path_c, bool complete) {
    if (pending_remote_reads.find(path_c) == pending_remote_reads.end()) {
        return;
    }

    START_LOG(gettid(), "call(bytes_received=%ld, offset=%ld, file_size=%ld, path_c=%s, complete=%s)",
