...
void handle_write(const char* str, int rank) {
        ...
        *std::get<1>(files_metadata[path]) = data_size; 
        ...
        #ifdef CAPIOLOG
        logfile << "handle write serving remote pending reads inside the loop" << std::endl;
        #endif
...
void handle_remote_read(char* str, char* p, int rank) {
	...
	*std::get<1>(files_metadata[path_c]) = offset + bytes_received;
	...
	c_file.insert_sector(offset, bytes_received);
...
void serve_remote_read(const char* path_c, int dest, long int offset, long int nbytes, int complete) {
	...
	sprintf(buf_send, "%s %s %s %s %s", s1, path_c, offset_cstr, nbytes_cstr, complete_cstr);
	#ifdef CAPIOLOG
...
void* wait_for_data(void* pthread_arg) {
	...
	free(rr_metadata);
			#ifdef CAPIOLOG
				logfile << "wait for data after" << std::endl;
			#endif
	return nullptr;
}
...
void* capio_helper(void* pthread_arg) {
	char* buf_recv = (char*) malloc(sizeof(char) * (PATH_MAX + 1024));
	MPI_Status status;
