...
#ifndef __CAPIO_POSIX //fix for older version of gcc found on galileo100 and leonardo
...
#ifndef CAPIO_MAX_LOG_LEVEL //capio max log level. defaults to -1, where everythong is logged
...
public:
    inline Logger(const char invoker[], const char file[], int line, long int tid, const char *message, ...) {


#ifndef __CAPIO_POSIX
        if (!logfile.is_open())
            logfile.open(std::string(CAPIO_SERVER_DEFAULT_LOG_FILE_NAME) + std::to_string(tid) + ".log",
                         std::ofstream::out);
#endif
        ...
        auto buf = reinterpret_cast<char *>(capio_syscall(SYS_mmap, nullptr, size + 1, PROT_READ | PROT_WRITE,
...
#define START_LOG(tid, message, ...) Logger log(__func__ , __FILE__, __LINE__, tid, message, ##__VA_ARGS__)
#else
