...
public:
    Circular_buffer(const std::string &shm_name, const long int _max_num_elems, const long int elem_size,
                    long int sem_timeout, int sem_retries) : _max_num_elems(_max_num_elems), _elem_size(elem_size), _shm_name(shm_name) {
        START_LOG(capio_syscall(SYS_gettid), "call(shm_name=%s, _max_num_elems=%ld, elem_size=%ld, sem_timeout=%d, sem_retries=%d)",
        ...
        _sem_num_elems = sem_open(("_sem_num_elems" + _shm_name).c_str(), O_CREAT | O_RDWR, S_IRUSR | S_IWUSR,
                                  0); //check the flags
        if (_sem_num_elems == SEM_FAILED) {
            ERR_EXIT("sem_open _sem_num_elems %s", _shm_name.c_str());
        }
        _sem_num_empty = sem_open(("_sem_num_empty" + _shm_name).c_str(), O_CREAT | O_RDWR, S_IRUSR | S_IWUSR,
    ...
    void read(T *buff_rcv) {
        ...
        if (sem_wait(_sem_num_elems) == -1) //wait for incoming message, without using the sem_wait_for
