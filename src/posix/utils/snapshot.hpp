...
void initialize_from_snapshot(const int *fd_shm, CPFiles_t *files, CPFileDescriptors_t *capio_files_descriptors,
                              CPFilesPaths_t *capio_files_paths, long tid) {
    START_LOG(tid, "call(%ld, %ld, %ld, %ld, %ld)", fd_shm, files, capio_files_descriptors, capio_files_paths, tid);
    int i = 0;
    std::string shm_name;
    int fd;
    off64_t *p_shm;
