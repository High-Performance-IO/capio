...
inline void register_data_listener(long tid) {
    auto *write_queue = new SPSCQueue("capio_write_data_buffer_tid_" + std::to_string(tid),
                                      CAPIO_DATA_BUFFER_LENGTH, CAPIO_DATA_BUFFER_ELEMENT_SIZE,
                                      get_capio_workflow_name());
    auto *read_queue =
        new SPSCQueue("capio_read_data_buffer_tid_" + std::to_string(tid), CAPIO_DATA_BUFFER_LENGTH,
