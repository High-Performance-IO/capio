...
void test_one_to_one(const std::string &buff_name, long int buff_size, long int num_elems, int rank,
...
void test_one_to_one_str(const std::string &buff_name, long int buff_size, long int num_elems,
                         int rank, sem_t *sem_prod, sem_t *sem_cons) {
...
sem_t *get_sem(const std::string sem_name) {
    sem_t *sem =
        sem_open(sem_name.c_str(), O_CREAT | O_RDWR, S_IRUSR | S_IWUSR, 0); // check the flags
    if (sem == SEM_FAILED) {
