...
void read_from_file(int* data, long int num_elements, long int num_reads, int rank, long int index) {
	...
	for (long int i = 0; i < num_reads; ++i) {
    	...
    	while (k < num_elements) {
        	long int num_elements_read = fread(data + i * num_elements + k, sizeof(int), (num_elements - k), fp);
        	k += num_elements_read;
			if (feof(fp)) {
...
void read_from_files(int* data, long int num_elements, long int num_reads, long int num_files, int rank, std::string time_file) {
	std::ofstream file;
