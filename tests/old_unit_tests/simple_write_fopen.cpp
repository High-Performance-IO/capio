...
void write_to_file(int* data, long int num_elements, long int num_writes, int rank, long int index, std::string time_file) {
	std::string file_name = "file_" + std::to_string(rank) + "_" + std::to_string(index) + ".txt";
	FILE* fp = fopen(file_name.c_str(), "wb");
	if (fp == NULL) {
		std::cerr << "impossible to open " << file_name << std::endl;
		exit(1);
	}
	for (long int i = 0; i < num_writes; ++i) {
    	...
    	while (k < num_elements) {
        	num_elements_written = fwrite(data + i * num_elements + k, sizeof(int), num_elements - k, fp);
...
void write_to_files(int* data, long int num_elements, long int num_writes, long int num_files, int rank, std::string time_file) {
	std::ofstream file;
