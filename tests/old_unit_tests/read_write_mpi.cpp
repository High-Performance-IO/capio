...
void reader(int rank, int master_rank, int *array, long int num_reads, std::size_t num_elements,
            int sender) {
    MPI_Status status;
    std::ofstream file;
    cclock::time_point before;
    ...
    for (long int i = 0; i < num_reads; ++i) {
        ...
        for (std::size_t k = 0; k < num_elements; k += num_elements_received) {
            if (num_elements - k > 1024L * 1024 * 1024 / sizeof(int)) {
                MPI_Recv(array + num_elements * i + k, 1024L * 1024 * 1024, MPI_INT, sender, 0,
