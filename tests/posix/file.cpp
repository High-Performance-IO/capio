...
TEST_CASE("Test file creation, reopen and close in a different directory using openat with absolute path", "[posix]") {
    const auto path_fs = std::filesystem::path(std::getenv("PWD")) / std::filesystem::path("test_file.txt");
...
TEST_CASE("Test file creation, reopen and close in a different directory using openat with dirfd", "[posix]") {
    constexpr const char* PATHNAME = "test_file.txt";
