...
int main(int argc, char** argv) {
    ...
    MPI_Alltoall(MPI_IN_PLACE, 0, MPI_INT, matrix[0], num_rows * num_cols / size, MPI_INT, MPI_COMM_WORLD);


    std::ofstream output_file("output_file_all_to_all_" + std::to_string(rank) + ".txt");
    for (int i = 0; i < num_rows; ++i) {
        for (int j = 0; j < num_cols; ++j) {
