...
CapioCLEngine *JsonParser::parse(const std::filesystem::path &source,
    ...
    server_println(CAPIO_LOG_SERVER_CLI_LEVEL_JSON, "Parsing configuration for workflow: " +
    ...
    for (auto app : io_graph) {
        ...
        if (app["input_stream"].get_array().get(input_stream)) {
            ...
            for (auto itm : input_stream) {
                ...
                server_println(CAPIO_LOG_SERVER_CLI_LEVEL_JSON, "Found file " + std::string(file));
    ...
    if (entries["storage"].get_object().get(storage_section)) {
        ...
        if (storage_section["memory"].get_array().get(storage_memory)) {
            ...
            for (auto file : storage_memory) {
                ...
                server_println(CAPIO_LOG_SERVER_CLI_LEVEL_JSON,
                               "Setting file " + std::string(file_str) + " to be stored in memory");
