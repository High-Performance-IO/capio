...
CapioCommunicationService::CapioCommunicationService(std::string &backend_name, const int port,
    ...
    if (backend_name == "TCP" || backend_name == "UCX") {
        ...
        capio_backend =
            new MTCLBackend(backend_name, std::to_string(port), CAPIO_BACKEND_DEFAULT_SLEEP_TIME);
        ...
        server_println(CAPIO_LOG_SERVER_CLI_LEVEL_INFO, "Skipping communication backend startup");
    } else {
