...
#include <include/storage-service/capio_file.hpp>
    ...
    void CapioStorageService::deleteFile(const std::string &file_name) const { _stored_files->erase(file_name); }
    ...
    void CapioStorageService::addThreadWaitingForData(pid_t tid, const std::string &path, capio_off64_t offset,
    ...
    void CapioStorageService::reply_to_client_raw(pid_t pid, const char *data, const capio_off64_t len) const {
        _server_to_client_queue->at(pid)->write(data, len);
    }


    void CapioStorageService::recive_from_client(pid_t tid, const std::string &file, capio_off64_t offset,
                            off64_t size) const {
